...
- 使用扩展入口 `openpi_ext/train.py`，它会：
  ...
  - 读取扩展配置（`openpi_ext.training.configs:get_configs()`）
...
- `openpi_ext/training/configs.py`：维护配置注册（`get_configs()`）
- `openpi_ext/training/data_configs.py`：自定义 DataConfigFactory
