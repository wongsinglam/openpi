...
def make_franka_ee_example() -> dict:
    """Creates a random input example for the Franka EE policy."""
    return {
        "observation.state": np.random.rand(7),  # 7维: 3 xyz + 3 rpy + 1 gripper
...
def _decode_video_frame(video_path: str, frame_index: int = 0) -> np.ndarray:
    ...
    if not Path(video_path).is_absolute():
        ...
        # List all directories that start with 'franka' or contain 'ee'
        possible_datasets = [d.name for d in lerobot_home.iterdir() if d.is_dir() and ('franka' in d.name.lower() or 'ee' in d.name.lower())]
...
class FrankaEEInputs(transforms.DataTransformFn):
    """
    This class is used to convert inputs to the model to the expected format for Franka EE space.
    ...
    def __call__(self, data: dict) -> dict:
        ...
        inputs = {
            "state": data["observation.state"],  # 7D: xyz + rpy + gripper
            "image": {
...
class FrankaEEOutputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # For Franka EE space, we return 7 actions (3 xyz + 3 rpy + 1 gripper).
        # EE space control: [x, y, z, roll, pitch, yaw, gripper]
